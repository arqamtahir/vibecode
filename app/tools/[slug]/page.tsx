...
import { toolWidgets } from "./widgets";
import { toolExplainers } from "./explainers";
...
export default async function ToolPage({ params }: { params: Promise<Params> }) {
  ...
  const Widget = toolWidgets[tool.slug];
  const explainer = toolExplainers[tool.slug];
  ...
  return (
    <ToolLayout
      ...
      explainer={
        explainer ? (
          <SeoExplainer heading={explainer.heading}>{explainer.body}</SeoExplainer>
        ) : (
